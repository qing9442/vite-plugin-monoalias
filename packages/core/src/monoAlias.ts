...
export default function monoAlias(options: ExternalAliasConfig = {}): Plugin {
  const { root, alias = { "@": "./src" }, debug } = options;
  ...
  return <Plugin>{
    ...
    async configResolved() {
      ...
      debug && console.log("[monorepo]", monorepo);
    },
    async resolveId(source: string, importer?: string, options?: any) {
      // 使用了别名的id
      const alia_config = aliasPrefix.find((alia) =>
        source.startsWith(`${alia}/`)
      );
      // console.log("===", source, importer, options);
      if (alia_config) {
        // 匹配上的别名，找到对应项目
        const findPackage = monorepo.packages.find((p) =>
          importer?.startsWith(p.node_path)
        );
        // options
        const src = new URL(alias[alia_config], `${findPackage?.node_path}/`);
        source = source.replace(`${alia_config}/`, `${src.href}/`);
        debug && console.log("[monorepo]alias", alia_config, source);
      }
